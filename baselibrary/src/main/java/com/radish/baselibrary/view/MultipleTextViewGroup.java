...
//        textSize = px2sp(context, textSize);
    ...
    public void setTextViews(final int count, final TextViewAdapter adapter) {
        this.mAdapter = adapter;
        if (layout_width >= 0) {
            setTextViewsTrue(count);
        } else {
    ...
    private void setTextViewsTrue(final int count) {
        ...
        int y = 0;
        TextView preTV = null;
        int textWMAX = layout_width - textPaddingLeft - textPaddingRight;
        for (int i = 0; i < count; i++) {
            final TextView tv = mAdapter.getTextView(i);
            if (tv == null) {
                continue;
            }
            ...
            tv.setOnClickListener(new OnClickListener() {
                ...
                public void onClick(View v) {
                    if (mAdapter != null) {
                        mAdapter.itemClick(tv, finalI, preTv, prePosition);
                    }
                    prePosition = finalI;
                    preTv = tv;
                }
            ...
            if (i != 0 && preTV != null) {
                ...
                paint.getTextBounds(preTV.getText().toString().trim(), 0, preTV.getText().toString().trim().length(), rect);
                int width = rect.width();//文字宽
                if (textWMAX < width) {
                    int hang = width / textWMAX;
                    if (width % textWMAX != 0) {
                        hang++;
                    }
                    tvh = tvh * hang - ((textPaddingTop + textPaddingBottom) * (hang - 1));
                }
            }
    ...
    public void clearSelect() {
        if (preTv != null) {
            if (mAdapter != null) {
                mAdapter.itemClick(null, -1, preTv, prePosition);
            }
            prePosition = -1;
            preTv = null;
        }
    ...
    public interface TextViewAdapter {
        TextView getTextView(int position);

        void itemClick(TextView tv, int position, TextView preTv, int prePosition);
    }
}
